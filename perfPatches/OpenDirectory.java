...
public abstract class OpenDirectory {
  ...
  public Directory mhWrap(Directory dir)
  {
      Class<?> clazz;
    try {
        clazz = Class.forName("org.apache.lucene.store.TimeLimitedDirectory");
        Constructor<?>[] constructors = clazz.getConstructors();
        dir=(Directory) constructors[0].newInstance(dir);
        System.out.println("\t\tUsing time limited directory...");
        
    } catch (Throwable e) {
        System.out.println("Error opening tld "+e);
    }
    return dir;  
  }
    ...
    if (dirImpl.equals("MMapDirectory")) {
      return new OpenDirectory() {
          ...
          public Directory open(File path) throws IOException {
            return mhWrap(new MMapDirectory(path));
          }
        };
      ...
      return new OpenDirectory() {
          @Override
          public Directory open(File path) throws IOException {
            return mhWrap(new NIOFSDirectory(path));
          }
        };
      ...
      return new OpenDirectory() {
          @Override
          public Directory open(File path) throws IOException {
            return mhWrap(new SimpleFSDirectory(path));
          }
        };
